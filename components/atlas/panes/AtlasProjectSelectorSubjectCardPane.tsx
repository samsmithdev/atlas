            ...
            <div className="flex flex-col flex-col-2 gap-2">
                {(projectNavigatorItems ?
                    (projectNavigatorItems.map((subject) => (
                        <AtlasSubjectWithProjectLinksCard subjectName={subject.header} subjectDescription={subject.description ?? ''} subjectId={subject.id} projects={subject.listItems} onDelete={() => subject.onDelete(subject.id)}/>
                    ))) : 
                    (<p></p>))}
            </div>
        </ScrollArea>
