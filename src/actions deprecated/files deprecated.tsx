...
export async function createFileFormTransactionDeprecated(
...
export async function createFileTransactionDeprecated(
  projectId: string,
  name: string,
  description: string,
  folderId?: string
...
export async function deleteFileDeprecated(fileId: string) {
  // Get the session inside the action
  const session = await auth();
  const userId = session?.user?.id;
